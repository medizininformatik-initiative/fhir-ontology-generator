...
class PathlingMappingGenerator(object):
    ...
    def generate_pathling_mapping(self, profile_snapshot, querying_meta_data: ResourceQueryingMetaData) \
            -> PathlingMapping:
        """
        Generates the pathling mapping for the given FHIR profile snapshot and querying meta data entry
        :param profile_snapshot: FHIR profile snapshot
        ...
        for attr_defining_id, attr_attributes in querying_meta_data.attribute_defining_id_type_map.items():
            ...
            attribute_fhir_path = self.translate_term_element_id_to_fhir_path_expression(attr_defining_id,
                                                                                         profile_snapshot)
            attribute = PathlingAttributeSearchParameter(attribute_type, attribute_key, attribute_fhir_path)
