...
class ElasticSearchGenerator:
    ...
    def __iterate_tree(self, ui_tree_list, term_code_info_map, target_elastic_crit_list,
                       context_termcode_hash_to_crit_set, namespace_uuid_str):
        ...
        for ui_tree in ui_tree_list:
            for entry in ui_tree['entries']:
                ...
                for parent_code in entry['parents']:
                    obj['parents'].append(
                        self.__get_relation_crit_object(parent_code, ui_tree['system'], context,
                                                        term_code_info_map, namespace_uuid_str))

                for child_code in entry['children']:
                    obj['children'].append(
                        self.__get_relation_crit_object(child_code, ui_tree['system'], context,
                                                        term_code_info_map, namespace_uuid_str))
    ...
    def __load_termcode_info(self, tree_file_name, namespace_uuid_str):
        term_code_info_map = {}
