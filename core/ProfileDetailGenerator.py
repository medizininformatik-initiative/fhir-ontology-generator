...
class ProfileDetailGenerator():
    ...
    def generate_detail_for_profile(self, profile):
        self.__logger.info(f"Generating profile detail for: {profile['url']}")

        if not "snapshot" in profile["structureDefinition"]:
            self.__logger.warning(f'profile with url {profile["url"]} has no snapshot - ignoring')
            return None
        ...
        for element in source_elements:
            ...
            if "type" in element:
                ...
                self.__logger.warning(f"Element without type: {element} - discarding")
                continue
            ...
            if field_type == "Reference" and len(self.get_referenced_mii_profiles(element, field_type)) == 0:
                self.__logger.warning(f"Discarding field: {element['id']} - No mii profiles that match referenced profiles, parent profile is {profile['url']}")
                continue
