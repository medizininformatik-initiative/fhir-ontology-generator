...
def configure_args_parser():
    arg_parser = argparse.ArgumentParser(
        description="Generate the consent for the MII-FDPG"
    )

    arg_parser.add_argument("--merge_mappings", action="store_true")
    arg_parser.add_argument("--project", required=True, help="Project to generate for")
...
def convert_bool_analysis_to_code(
    distributed_analysis, eu_gdpr, insurance_data, contact
) -> str:
    return "-".join([distributed_analysis, eu_gdpr, insurance_data, contact])


def generate_fixed_fhir_criteria(
    provisions_code, provisions_display
) -> list[FixedFHIRCriteria]:
    return [
        FixedFHIRCriteria(
            type="coding",
...
def generate_fixed_cql_criteria(
    provisions_code, provisions_display
) -> list[FixedCQLCriteria]:
    return [
        FixedCQLCriteria(
    ...
    with open(csv_file, mode="r", newline="", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            cql_mapping.timeRestriction = CQLTimeRestrictionParameter(
                path="datetime", types={"dateTime"}, cardinality=SimpleCardinality.MANY
            )
            ...
            provisions_code = [
                code.strip() for code in row["provisions-code"].split("|")
            ]
            provisions_display = [
                display.strip() for display in row["provisions-display"].split("|")
            ]

            fhir_mapping.fixedCriteria = generate_fixed_fhir_criteria(
                provisions_code, provisions_display
            )
            cql_mapping.fixedCriteria = generate_fixed_cql_criteria(
                provisions_code, provisions_display
            )
            ...
            consent_mapping_tree.entries[consent_code] = TermEntryNode(
                term_code=term_code
            )
...
def generate_js_lookup_table(lookup_table: dict) -> str:
    ...
    js_lookup_table += "".join(
        f'  "{key.lower()}": {json.dumps(value.to_dict())},\n'
...
if __name__ == "__main__":
    ...
    consents_fhir, consents_cql, consent_mapping_tree, lookup_table = process_csv(
        input_file
    )
    ...
    save_json(
        consent_output_dir / "consent-mappings-tree.json",
        consent_mapping_tree.to_dict(),
    )

    logger.info("Generating lookup table")
    with open(
        consent_output_dir / "consent-js-lookup-table.js", mode="w+", encoding="UTF-8"
    ) as f:
    ...
    if args.merge_mappings:
        ...
        append_to_json(
            output_dir / "cql" / "mapping_cql.json",
            output_dir / "cql" / "mapping_cql.json",
            consents_cql,
        )
        append_to_json(
            output_dir / "fhir" / "mapping_fhir.json",
            output_dir / "fhir" / "mapping_fhir.json",
            consents_fhir,
        )
        append_to_json(
            output_dir / "mapping_tree.json",
            output_dir / "mapping_tree.json",
            [consent_mapping_tree.to_dict()],
        )
