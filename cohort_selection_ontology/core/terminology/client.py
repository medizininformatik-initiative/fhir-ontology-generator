...
class CohortSelectionTerminologyClient(FhirTerminologyClient):
    ...
    def get_closure_map(
        self, term_codes: Iterable[TermCode], closure_name: str = "closure-test"
    ) -> ConceptMap:
        """
        Returns the closure map of a set of term codes. If the set contains codes from multiple versions of the same
        code system only codes from the most recent version are included
        ...
        grouped_term_codes = [
            ...
            for g1 in groupby(
                sorted(term_codes, key=lambda t: t.system), lambda t: t.system
            )
        ]

        included_term_codes = []
        for system, version_groups in grouped_term_codes:
            if len(version_groups) > 1:
                versions = version_groups.keys()
                most_recent_version = sorted(versions)[-1]
                self.__logger.warning(
                    f"Value set supports multiple versions {sorted(versions)} of code system [url='{system}'] "
                    f"=> Proceeding with most recent version '{most_recent_version}'"
                )
                included_term_codes.extend(version_groups[most_recent_version])
            else:
                included_term_codes.extend(next(iter(version_groups.values())))

        parameters = Parameters(
            parameter=[ParametersParameter(name="name", valueString=closure_name)]
        )
        for term_code in included_term_codes:
            value_coding = Coding(
