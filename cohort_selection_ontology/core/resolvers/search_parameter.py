...
class SearchParameterResolver(ABC):
    ...
    search_parameters: List[dict] | None = field(default_factory=list)

    def __init__(self, **data):
        super().__init__(**data)
        self.search_parameters = self._load_all_search_parameters()

    def find_composite_search_parameter(
        ...
        if len(search_parameters) != 2:
            raise ValueError(
                "Only two search parameters are allowed for composite search"
            )
        search_parameter_url_1, search_parameter_url_2 = [
            param.get("url") for param in search_parameters.values()
        ]
        path_1, path_2 = search_parameters.keys()
        joined_path = f"{path_1}.where({path_2})"
        for composite_search_parameter in [
            search_parameter
            for search_parameter in self.search_parameters
            if search_parameter.get("type") == "composite"
        ]:
            components = composite_search_parameter.get("component")
            if (
                components[0].get("definition") == search_parameter_url_1
                and components[1].get("definition") == search_parameter_url_2
            ):
                return orderedDict({joined_path: composite_search_parameter})
            if (
                components[0].get("definition") == search_parameter_url_2
                and components[1].get("definition") == search_parameter_url_1
            ):
                return orderedDict({joined_path: composite_search_parameter})
        raise ValueError("Composite search parameter not found")

    def find_search_parameter(
        self, fhir_path_expressions: List[str]
        ...
        fhir_path_expressions = [
            expression
            for expression in fhir_path_expressions
            if not expression.startswith("Extension")
        ]
        fhir_path_expressions = [
            re.sub(r"^\((.*)\)$", r"\1", expression)
            for expression in fhir_path_expressions
        ]
        fhir_path_expressions_to_search_parameter: OrderedDict[
            str, Tuple[dict, int]
        ] = orderedDict(
        ...
        try:
                ...
                for path_expression in fhir_path_expressions:
                    if path_expression in expressions:
                        ...
                        number_of_relevant_expressions = len(
                            list(
                                filter(
                                    lambda x: x.startswith(resource_type), expressions
                                )
                            )
                        )
                        if (
                            not fhir_path_expressions_to_search_parameter.get(
                                path_expression
                            )
                            or number_of_relevant_expressions
                            < fhir_path_expressions_to_search_parameter.get(
                                path_expression
                            )[1]
                        ):
                            fhir_path_expressions_to_search_parameter[
                                path_expression
                            ] = (search_parameter, number_of_relevant_expressions)
            result = orderedDict(
                [
                    (key, value[0])
                    for key, value in fhir_path_expressions_to_search_parameter.items()
                ]
            )
            if missing_search_parameters := [
                ...
                if any(
                    [
                        " as " in fhir_path_expressions
                        and "." not in fhir_path_expressions.split("as")[1]
                        for fhir_path_expressions in missing_search_parameters
                    ]
                ):
                    result_without_as_cast = self.find_search_parameter(
                        [
                            fhir_path_expressions.split(" as ")[0]
                            for fhir_path_expressions in missing_search_parameters
                        ]
                    )
                    result = orderedDict(
                        [
                            (
                                ...
                                (
                                    ...
                                    if key.split(" as ")[0]
                                    not in result_without_as_cast
                                    else result_without_as_cast[key.split(" as ")[0]]
                                ),
                            )
                    ...
                    if set(new_paths) == set(missing_search_parameters):
                        raise RecursionError(
                            "Exhausted all paths up to their base node without finding a result"
                        )
                    ...
                    for path in result.keys():
                        for shortened_path in result_without_last_path_element.keys():
                            if shortened_path in path:
                                result[shortened_path] = (
                                    result_without_last_path_element[shortened_path]
                                )
                                result.pop(path)
            ...
            raise NotFoundError(
                f"Could not find search parameter for expressions {fhir_path_expressions}"
            ) from err
    ...
    def _load_default_search_parameters() -> List[Dict]:
        with (
            files(fhir_resource_files)
            .joinpath("fhir_search_parameter_definition.json")
    ...
    def get_cleaned_expressions(self, search_parameter: dict) -> List[str]:
        ...
        expressions = [
            (
                self.translate_as_function_to_operand(expression)
                if not re.match(r"\((.*?)\)", expression)
                else self.translate_as_function_to_operand(expression[1:-1])
            )
            for expression in expressions.split(" | ")
        ]
        expressions = [
            self.convert_of_type_to_as_operand(expression) for expression in expressions
        ]
        return expressions
    ...
    def translate_as_function_to_operand(expression) -> str:
        ...
        while "as(" in expression:
            ...
            expression = (
                expression[:as_start]
                + " as "
                + expression[as_start + 4 : as_end]
                + expression[as_end + 1 :]
            )
            if len(expression) > as_end + 1 and expression[as_end + 1] == ".":
                Exception(
                    "Not implemented: Todo: Put operation in round brackets. "
