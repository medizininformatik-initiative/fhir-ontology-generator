...
class CQLMappingGenerator(object):
    ...
    def generate_normalized_term_code_cql_mapping(
        self, profile_snapshot, module_name: str
    ) -> Tuple[Dict[Tuple[TermCode, TermCode], str], Dict[str, CQLMapping]]:
        """
        ...
        if tc_defining_id := querying_meta_data.term_code_defining_id:
            ...
            if (
                ...
                if is_element_in_snapshot(profile_snapshot, tc_defining_id):
                    element = get_element_from_snapshot(
                        profile_snapshot, tc_defining_id
                    )
                else:
                    element = get_element_defining_elements(
                        tc_defining_id, profile_snapshot, module_dir_name, modules_dir
                    )[-1]
                ...
                cql_mapping.termCode = CQLTypeParameter(
                    term_code_fhir_path, types, card
                )
    ...
    def get_composite_code(self, attribute, profile_snapshot, module_dir_name):
        ...
        attribute_parsed = get_element_defining_elements(
            attribute, profile_snapshot, module_dir_name, modules_dir
        )
    ...
    def get_composite_attribute_type(
        self, attribute, profile_snapshot, module_dir_name
    ):
        modules_dir = self.__project.input.cso.mkdirs("modules")
        attribute_parsed = get_element_defining_elements(
            attribute, profile_snapshot, module_dir_name, modules_dir
    ...
    def translate_composite_attribute_to_fhir_path_expression(
        self, attribute, profile_snapshot, module_dir_name: str
    ):
        modules_dir = self.__project.input.cso.mkdirs("modules")
        ...
        expressions = translate_element_to_fhir_path_expression(
            elements, profile_snapshot, is_composite=True
        )
    ...
    def translate_term_element_id_to_fhir_path_expression(
        self, element_id, profile_snapshot, module_dir_name: str
    ) -> str:
        modules_dir = self.__project.input.cso.mkdirs("modules")
        elements = get_element_defining_elements(
            element_id, profile_snapshot, module_dir_name, modules_dir
        )
    ...
    def translate_element_id_to_fhir_path_expressions(
        self, element_id, profile_snapshot: dict, module_dir_name: str
    ) -> str:
        """
    ...
    def get_attribute_type(
        self, profile_snapshot: dict, attribute_id: str, module_dir_name: str
    ) -> VALUE_TYPE_OPTIONS:
        """
        ...
        attribute_type = extract_value_type(
    ...
    def get_reference_type(
        self, profile_snapshot: dict, attr_defining_id, module_dir_name: str
    ):
        """
        Returns the type of the given attribute
        :param profile_snapshot: FHIR profile snapshot
        ...
        modules_dir = self.__project.input.cso.mkdirs("modules")
        elements = get_element_defining_elements(
            attr_defining_id, profile_snapshot, module_dir_name, modules_dir
        )
        for element in elements:
            for element_type in element.get("type"):
                if element_type.get("code") == "Reference":
                    return extract_reference_type(
                        element_type, modules_dir, profile_snapshot.get("name")
                    )
    ...
    def __aggregate_cardinality_using_element_id(
        self, element_defining_id: str, profile_snapshot: Snapshot, module_dir_name: str
    ) -> SimpleCardinality:
        modules_dir = self.__project.input.cso.mkdirs("modules")
        element_results = get_element_defining_elements_with_source_snapshots(
            element_defining_id, profile_snapshot, module_dir_name, modules_dir
        )
