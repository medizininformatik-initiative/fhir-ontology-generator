...
class PathlingMappingGenerator(object):
    ...
    def generate_mapping(
        ...
        full_context_term_code_pathling_mapping_name_mapping: (
            Dict[Tuple[TermCode, TermCode]] | dict
        ) = {}
        full_pathling_mapping_name_pathling_mapping: (
            Dict[str, PathlingMapping] | dict
        ) = {}
        for module_dir in [
            folder for folder in os.scandir(fhir_dataset_dir) if folder.is_dir()
        ]:
            self.module_dir: str = module_dir.path
            files = [
                file.path
                for file in os.scandir(f"{fhir_dataset_dir}/{module_dir.name}/package")
            ...
            for file in files:
                with open(file, "r", encoding="utf8") as f:
                    ...
                    context_tc_to_mapping_name, pathling_mapping_name_to_mapping = (
                        self.generate_normalized_term_code_pathling_mapping(
                            snapshot, module_dir.name
                        )
                    )
                    full_context_term_code_pathling_mapping_name_mapping.update(
                        context_tc_to_mapping_name
                    )
                    full_pathling_mapping_name_pathling_mapping.update(
                        pathling_mapping_name_to_mapping
                    )
        return (
            full_context_term_code_pathling_mapping_name_mapping,
            full_pathling_mapping_name_pathling_mapping,
    ...
    def generate_normalized_term_code_pathling_mapping(
        ...
        for querying_meta_data_entry in querying_meta_data:
            if querying_meta_data_entry.name not in self.generated_mappings:
                pathling_mapping = self.generate_pathling_mapping(
            ...
            term_codes = (
                ...
                else profile_snapshot.get_term_code_by_id(
    ...
    def generate_pathling_mapping(
        ...
        if tc_defining_id := querying_meta_data.term_code_defining_id:
            pathling_mapping.termCodeFhirPath = (
        ...
        if val_defining_id := querying_meta_data.value_defining_id:
            pathling_mapping.valueFhirPath = (
                self.translate_element_id_to_fhir_path_expressions(
                    val_defining_id, profile_snapshot
                )
            )
            pathling_mapping.valueType = self.get_attribute_type(
                profile_snapshot, val_defining_id
            )
        if time_defining_id := querying_meta_data.time_restriction_defining_id:
            pathling_mapping.timeRestrictionFhirPath = (
                self.translate_element_id_to_fhir_path_expressions(
        ...
        for (
            ...
            attribute_type = (
                attr_type
                if attr_type
                else self.get_attribute_type(profile_snapshot, attr_defining_id)
            )
            # FIXME:
            ...
            attribute = PathlingAttributeSearchParameter(
                types=attribute_type,
                key=attribute_key,
                attributePath=attribute_fhir_path,
            )
    ...
    def translate_element_id_to_fhir_path_expressions(
        ...
        return ".".join(
            [
                self.get_pathling_optimized_path_expression(expression)
    ...
    def get_attribute_type(
        self, profile_snapshot: StructureDefinitionSnapshot, attribute_id: str
    ) -> VALUE_TYPE_OPTIONS:
        """
        ...
        attribute_element = resolve_defining_id(
            profile_snapshot, attribute_id, self.data_set_dir, self.module_dir
        )
