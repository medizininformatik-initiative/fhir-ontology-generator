...
from common.util.terminology.ValueSetResolver import get_termcodes_from_onto_server
...
class UIProfileGenerator:
    ...
    def generate_normalized_term_code_ui_profile_mapping(self, profile_snapshot: dict, module_name) \
            -> Tuple[Dict[Tuple[TermCode, TermCode], str], Dict[str, UIProfile]]:
        """
        Generates a generators from term codes to UI profiles
        :param module_name: name of the module the profile belongs to
        :param profile_snapshot: FHIR profile snapshot
        :return: Tuple of the normalized tables to obtain the generators from term code + context to UI profile
    ...
    def get_value_definition(self, profile_snapshot, querying_meta_data) -> ValueDefinition:
        ...
        # for the generators but the UI profile only supports "concept" so we have to convert it here.
        if value_type == "code":
            value_type = "concept"
