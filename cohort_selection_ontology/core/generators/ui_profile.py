...
class UIProfileGenerator:
    ...
    def get_value_definition(
        ...
        if value_type == "concept":
            value_definition.referencedValueSet = get_selectable_concepts(
    ...
    def get_attribute_definition(
        ...
        if len(attribute_defining_elements) > 1:
            self.__logger.warning(
                "More than one attribute definition element but only one is supported => Using last one"
            )
    ...
    def get_reference_criteria_set(
        ...
        if fixed_term_codes := get_fixed_term_codes(
            element, snapshot, module_dir, self.data_set_dir, self.__client
        ):
            return self.get_reference_criteria_set_from_fixed_term_codes(
            ...
            raise Exception(
                "Unable to generate reference criteria set for element: "
                + element.get("id")
                + " in profile: "
                + snapshot.get("name")
            )
