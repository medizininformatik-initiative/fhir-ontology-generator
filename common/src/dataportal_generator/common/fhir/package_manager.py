...
from dataportal_generator.common.util.codec.json import load_json
from dataportal_generator.common.http.client import BaseClient
...
class FhirPackageManager(abc.ABC):
    ...
    ) -> Optional[IdxStructureDefinition]:
        """
        Searches for the profile identified by the provided URL. Either ``url`` or ``name`` has to be provided

        :param url: URL string value to match against ``StructureDefinition.url``
        :param name: String value to match against ``StructureDefinition.name``
        :param version: (Optional) Profile version, e.g. ``StructureDefinition.version`` value
        :return: ``StructureDefinition`` instance if found, else ``None``
        """
        if url:
            pattern = {"url": url}
        elif name:
            ...
            raise ValueError("Either 'url' or 'name' has to be provided")
        if version:
            pattern["version"] = version
        return self.find(index_pattern=pattern, latest_only=False)
