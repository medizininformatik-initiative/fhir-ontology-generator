...
def load_bundle_onto_fhir_server(fhir_api: str, bundle: dict) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(
                f"Request failed with status code: {response.status_code} because {response.reason}"
            )
            return False
...
def load_list_of_resources_onto_fhir_server(
    fhir_api: str, files: list[str], testdata_folder: str
):
    resource_bundle = create_bundle(BundleType.TRANSACTION)
    for resource_file in files:
        with open(
            os.path.join(testdata_folder, resource_file), "r", encoding="utf-8"
        ) as f:
            json_data = json.loads(f.read())

        resource_url = json_data.get("resourceType") + "/" + json_data.get("id")
        resource_bundle["entry"].append(
            {"resource": json_data, "request": {"method": "PUT", "url": resource_url}}
        )
...
def delete_list_of_resources_from_fhir_server(fhir_api: str, fhir_resources: list[str]):
    ...
    for resource in fhir_resources:
        ...
        resource_bundle["entry"].append(
            {"request": {"method": "DELETE", "url": resource_url}}
        )
    return load_bundle_onto_fhir_server(fhir_api, resource_bundle)


def check_response_bundle(
    bundle: dict, expected_status_codes: set[int]
) -> Tuple[bool, Optional[OperationOutcome]]:
    """
    ...
    if bundle_type not in allowed_bundle_types:
        logger.warning(
            f"Bundle type has to be one of {allowed_bundle_types} to be eligible for validation"
        )
        return True, None
    ...
    for idx, entry in enumerate(bundle.get("entry", [])):
        ...
        if response is None:
            oo.issue.append(
                OperationOutcomeIssue(
                    severity="warning",
                    code="processing",
                    diagnostics="No 'response' element in bundle entry",
        ...
        if response_oo_json:
            ...
            if any(
                issue.severity in {"error", "fatal"} for issue in response_oo.issue
            ):  # Is any issue severe?
                valid = False
    return valid, oo
