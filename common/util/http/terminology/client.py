...
from common.util.fhir.parameters import summarize_parameters
...
class FhirTerminologyClient(BaseClient):
    ...
    def expand_value_set(
        ...
        try:
            ...
            raise Exception(
                f"Failed to expand value set [url='{url}', version='{version}']"
            ) from exc
    ...
    def closure(self, parameters: Parameters) -> ConceptMap:
        try:
            ...
            raise Exception(
                f"Failed to update closure table [{summarize_parameters(parameters, {'name': 'valueString', 'version': 'valueString'})}]"
            ) from exc
