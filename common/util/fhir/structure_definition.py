...
ElementDefinitionDict = Annotated[
    Mapping[str, Any], "Dictionary representing FHIR ElementDefinition instance"
]
StructureDefinitionDict = Annotated[
    Mapping[str, Any], "Dictionary representing FHIR StructureDefinition instance"
]
Differential = Annotated[
    StructureDefinitionDict,
    (
        "Dictionary representing FHIR StructureDefinition instance as a "
        "differential view"
    ),
]
Snapshot = Annotated[
    StructureDefinitionDict,
    "Dictionary representing FHIR StructureDefinition instance as a snapshot view",
]


def get_element_from_snapshot(profile_snapshot, element_id) -> Optional[Dict]:
    """
    ...
    :return: Dictionary representing `ElementDefinition` instance with matching ID value in snapshot or `None` if no ID
             matched
    """
    if not profile_snapshot.get("snapshot"):
        raise KeyError(
            f"Assumed snapshot has no snapshot element [name='{profile_snapshot.get('name')}']"
        )
    for element in profile_snapshot.get("snapshot", {}).get("element", []):
...
def get_parent_element(
    element: ElementDefinitionDict, snapshot: Snapshot
) -> Optional[ElementDefinitionDict]:
    element_id = element.get("id")
    if not element_id:
        raise KeyError(
            f"'ElementDefinition.id' is missing in element [path='{element.get('path')}']"
        )
    ...
    parents = list(
        filter(
            lambda e: e.get("id") == parent_id,
            snapshot.get("snapshot", {}).get("element", []),
        )
    )
    match len(parents):
        case 0:
        ...
        case _:
            raise Exception(
                f"More than one parent element was identified [id='{parent_id}']"
            )
...
def find_type_element(
    element: ElementDefinitionDict, fhir_type: FhirDataType
