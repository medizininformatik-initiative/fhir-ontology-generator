...
class StructureDefinitionParserTestCase(unittest.TestCase):
    ...
    def test_get_element_defining_elements(self):
        with open(
            ...
            "FDPG_Observation_Digitoxin-snapshot.json",
            "r",
        ) as f:
            profile = json.load(f)
            for element in profile.get("snapshot").get("element"):
                ...
                if not "." in element_id:
                    continue
                print(element_id)
                resolved_element = get_element_defining_elements(
                    element_id,
                    profile,
    ...
    def test_get_element_defining_elements_complex(self):
        with open(
            ...
            profile = json.load(f)
            resolved_element = get_element_defining_elements(
                "((Specimen.extension:festgestellteDiagnose).value[x])"
                ".code.coding:icd10-gm",
                profile,
                "../../projects/mii_core_data_set/resources/"
