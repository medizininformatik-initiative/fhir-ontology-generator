...
def validate_json_object(json_content: dict, json_schema: dict, schema_store: dict = None, json_file: Path | bool = 0,
                         schema_file: Path = 0):
    def handler(uri: str):
    ...
    try:
        ...
        if json_file != 0:
            ...
            loc = pointer_map.get(pointer)  # clickable in many IDEs/terminals - id does not work in PyCharm
...
def test_criterion_definition_validity(querying_metadata: ResourceQueryingMetaData, querying_metadata_schema: dict):
    assert validate_json_object(json.loads(querying_metadata.to_json()), querying_metadata_schema)
...
def test_json_integrity_profile_tree(test_dir, project, schema_store):
    ...
    assert validate_json_with_file(file=profile_tree_file, schema=profile_tree_schema_file,
...
def test_json_integrity_term_code_info(test_dir, project, schema_store):
    ...
    term_code_info_schema_file = Path(test_dir, "schemata", "term_code_info_schema.json")

    for term_code_file in os.listdir(term_code_info_folder):
        term_code_file = term_code_info_folder / term_code_file
        assert validate_json_with_file(file=Path(term_code_file), schema=term_code_info_schema_file,
                                               schema_store=schema_store)
...
def test_json_integrity_ui_trees(test_dir, project, schema_store):
    ...
    for ui_tree_file in os.listdir(ui_tree_folder):
        ...
        assert validate_json_with_file(file=ui_tree_file, schema=ui_tree_schema_file, schema_store=schema_store)
...
def test_json_integrity_criteria_sets(test_dir, project, schema_store):
    ...
    for ui_tree_file in os.listdir(criteria_sets):
        ...
        assert validate_json_with_file(file=ui_tree_file, schema=criteria_sets_schema_file, schema_store=schema_store)
...
def test_json_integrity_mapping_cql(test_dir, project, schema_store):
    ...
    assert validate_json_with_file(file=mapping_cql, schema=mapping_cql_schema_file, schema_store=schema_store)
...
def test_json_integrity_mapping_fhir(test_dir, project, schema_store):
    ...
    assert validate_json_with_file(file=mapping_fhir, schema=mapping_fhir_schema_file,
                                           schema_store=schema_store)
...
def test_json_integrity_mapping_tree(test_dir, project, schema_store):
    ...
    assert validate_json_with_file(file=mapping_tree, schema=mapping_tree_schema_file,
                                           schema_store=schema_store)
...
def test_json_integrity_mapping_tree_dse(test_dir, project, schema_store):
    ...
    mapping_tree_dse_schema_file = Path(test_dir, "schemata", "mapping_tree_dse_schema.json")
...
def test_json_integrity_elastic_file(json_file, test_dir, schema_store):
    ...
    validate_content = fastjsonschema.compile(elastic_ontology_content_schema_file, handlers={"https": handler})
    validate_index = fastjsonschema.compile(elastic_ontology_index_schema_file, handlers={"https": handler})
