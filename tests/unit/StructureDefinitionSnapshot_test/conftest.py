...
def __sample_snapshot_bioprobe_str() -> str:
    with open(os.path.join(__test_dir(), "testdata", "FDPG_Bioprobe-snapshot.json"), "r", encoding="UTF-8") as f:
        return f.read()
...
def sample_snapshot_bioprobe() -> StructureDefinitionSnapshot:
    return StructureDefinitionSnapshot.model_validate_json(__sample_snapshot_bioprobe_str())

@pytest.fixture(scope="session")
def sample_snapshot_diagnose() -> StructureDefinitionSnapshot:
    return StructureDefinitionSnapshot.model_validate_json(__sample_snapshot_diagnose_str())
