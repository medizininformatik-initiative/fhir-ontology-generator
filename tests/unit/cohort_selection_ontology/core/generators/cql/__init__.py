...
def select_element_compatible_with_cql_operations(
    ...
    if "sliceName" in element:
        return select_element_compatible_with_cql_operations(
            get_parent_element(element, snapshot), snapshot
        )
    ...
    if len(element_types) == 1 and "Coding" in element_type_codes:
        ...
        if element_base_path:
            ...
            raise KeyError(
                f"Element [id='{element.get('id')}'] is missing required 'ElementDefinition.base.path' "
        ...
        case _:
            ...
            match parent_element:
                case None:
                    if not is_root:
                        raise Exception(
                            f"No parent could be identified for element '{opt_element.get('id')}' with "
...
def aggregate_cardinality_of_element_chain(
    chain: List[Tuple[Snapshot, ElementDefinitionDict]],
    card_type: Literal["min", "max"] = "max",
) -> SimpleCardinality:
    """
    ...
    return foldl(
        ...
        lambda acc, t: acc if acc == SimpleCardinality.MANY else acc * aggregate_cardinality_using_element(t[1], t[0], card_type),
    )
