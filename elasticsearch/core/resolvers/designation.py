...
class TerminologyDesignationResolver:
    ...
    def __init__(
        self,
        project: Project,
        base_translations_conf: str | Path = None,
    ...
    def __optimize_code_system_concepts(code_system_content):
        ...
        for concept in code_system_content.get("concept"):
            ...
            for designation in concept.get("designation"):
                temp[concept.get("code")][designation.get("language")] = (
                    designation.get("value")
                )
        return temp
    ...
    def __load_base_translations(self, base_translation_conf: str | Path):
        ...
        """
        self.__logger.debug(
            f"Loading base translation config located @ {base_translation_conf}"
        )
        ...
        try:
            with open(base_translation_conf, mode="r", encoding="UTF-8") as conf_file:
                self.base_translation_mapping.update(
                    json.load(conf_file).get("code_system_translations", {})
                )
        except OSError as exc:
            self.__logger.warning(
                f"Failed to load base translation config @ {base_translation_conf}",
                exc_info=exc,
                stack_info=True,
            )
    ...
    def __batch_lookup(self, bundle: dict) -> dict:
        ...
        for bundle_chunk in split_bundle(bundle, self.max_bundle_size):
            ...
            if response.status_code != 200:
                raise Exception(
                    f"Unexpected status code [actual={response.status_code}, expected=200]. "
    ...
    def __save_bundle_content(
        self, bundle: dict, system_url: str, languages: List[str]
    ):
        """
            ...
            if not resource:
                ...
                match resource["resourceType"]:
                    ...
                    case _:
                        failure_cnt += 1
                        self.__logger.debug(
                            f"Unexpected resource type '{resource['type']}' => Skipping"
                        )
        if failure_cnt > 0:
            self.__logger.warning(
                f"{failure_cnt}/{len(bundle.get('entry', []))} requests failed. Change log level to "
                f"'debug' to view details"
            )
    ...
    def __load_base_designations_for_ui_tree(self, ui_tree: dict):
        ...
        request_part = {"method": "POST", "url": "CodeSystem/$lookup"}
        # Process contained trees for each system
        for system_tree in ui_tree:
            system = system_tree.get("system")
            if system is None:
                self.__logger.warning(
                    "Tree has no system URL and thus no look up can be performed. Skipping"
                )
                continue
            ...
            if parameters_template is None:
                self.__logger.debug(
                    f"No parameters template defined for system [url={system}]. Skipping"
                )
                continue
            self.__logger.debug(
                f"Processing system tree [url={system}, " f"version={version}]"
            )
            ...
            for entry in system_tree.get("entries", []):
                ...
                parameters.get("parameter").extend(
                    [
                        {"name": "code", "valueCode": entry["key"]},
                        {"name": "system", "valueUri": system},
                    ]
                )
                bundle["entry"].append(
                    {"resource": parameters, "request": request_part}
                )
    ...
    def __load_base_designations_for_value_set(self, value_set: dict):
        ...
        """
        self.__logger.debug(
            f"Loading base designations for value set [url={value_set.get('url')}]"
        )
        request_part = {"method": "POST", "url": "CodeSystem/$lookup"}
    ...
    def load_base_designations(
        self, ui_tree_dir: str | Path = None, value_set_dir: str | Path = None
    ):
        """
        ...
        if ui_tree_dir is None:
            ...
            ui_trees = [
                json.load(
                    open(
                        os.path.join(ui_tree_dir, file_name), mode="r", encoding="UTF-8"
                    )
                )
        ...
        if value_set_dir:
            ...
            value_sets = [
                json.load(
                    open(
                        os.path.join(value_set_dir, file_name),
                        mode="r",
                        encoding="UTF-8",
                    )
                )
                for file_name in os.listdir(value_set_dir)
                if file_name.endswith(".json")
            ]
        if not self.base_translation_mapping:
            self.__logger.warning(
                "No mapping seems to exist and thus no designations can be loaded. Aborting"
            )
        else:
    ...
    def load_designations(
        self, folder_path: str | Path, update_translation_supplements=False
    ):
        """
        ...
        if update_translation_supplements:
            ...
            supplement_registry = self.__client.get_code_system(
                "fdpg-plus-translation-supplement-registry"
            )
            if supplement_registry is None:
                self.__logger.warning(
                    "Could not find supplement registry code system on the terminology server "
                    "=> Skipping update"
                )
            else:
                for code_system_concept in supplement_registry.concept:
                    for prop in code_system_concept.property:
                        if prop.code == "supplement-canonical":
                            ...
                            supplement_code_system_bundle = (
                                self.__client.search_code_system(
                                    url=code_system_url, version=code_system_version
                                )
                            )
                            if not supplement_code_system_bundle.entry:
                                self.__logger.warning(
                                    "No entry was found in this bundle => Skipping "
                                    + code_system_url
                                    + "|"
                                    + code_system_version
                                )
                            ...
                            supplement_code_system_full_url = (
                                supplement_code_system_bundle.entry[0].fullUrl
                            )
                            self.__logger.info(
                                "Downloading: " + supplement_code_system_full_url
                            )
                            response_cs_content = self.__client.get(
                                full_url=supplement_code_system_full_url,
                            ...
                            if response_cs_content.status_code != 200:
                                self.__logger.warning(
                                    ...
                                    + response_cs_content.status_code
                                    + ". Expected 200"
                                )
                                self.__logger.debug(
                                    "Content: " + response_cs_content.content
                                )
                                continue

                            actual_code_system = response_cs_content.json()
                            actual_code_system_name = actual_code_system.get("name")
                            file_path_save_code_system = os.path.join(
                                folder_path, actual_code_system_name + ".json"
                            )
                            with open(
                                file_path_save_code_system, mode="w", encoding="UTF-8"
                            ) as f:
        ...
        for codesystem_file in sorted(codesystem_files):
            if codesystem_file.endswith(".json"):
                self.__logger.debug(
                    f"Processing CodeSystem supplement file {codesystem_file}"
                )
                with open(
                    os.path.join(folder_path, codesystem_file),
                    mode="r",
                    encoding="UTF-8",
                ) as json_file:
                    ...
                    codesystem["concept"] = self.__optimize_code_system_concepts(
                        codesystem
                    )
                    ...
                    if self.code_systems.get(url):
                        for (
                            new_concept_code,
                            new_concept_designations,
                        ) in codesystem.get("concept").items():
                            existing_concept = self.code_systems[url]["concept"].get(
                                new_concept_code
                            )

                            if existing_concept:
                                if (
                                    "de" not in existing_concept
                                    and "de" in new_concept_designations
                                ):
                                    existing_concept["de"] = (
                                        new_concept_designations.get("de")
                                    )
                                if (
                                    "en" not in existing_concept
                                    and "en" in new_concept_designations
                                ):
                                    existing_concept["en"] = (
                                        new_concept_designations.get("en")
                                    )
                            else:
                                self.code_systems[url]["concept"][
                                    new_concept_code
                                ] = new_concept_designations
                    else:
    ...
    def resolve_term(self, term_code) -> Mapping[str, Any]:
        ...
        if self.code_systems.get(term_code["system"]):
            concept = (
                self.code_systems.get(term_code["system"])
                .get("concept")
                .get(term_code["code"])
            )
            if concept:
                if "en" in concept:
                    ...
                    if concept.get("en") is None:
                        self.__logger.warning(
                            f"Did not find English version of code '{term_code['code']}'"
                        )
                if "de" in concept:
                    display["de"] = concept.get("de")
                    if concept.get("de") is None:
                        self.__logger.warning(
                            f"Did not find German version of code '{term_code['code']}'"
                        )
        else:
            self.__logger.debug(
                f"Did not find code system '{term_code['system']}' [requested_code='{term_code['code']}']"
            )
        return display
