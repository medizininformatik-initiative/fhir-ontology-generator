...
def __validate_project_template(template_dir: Path):
    try:
        ...
        if not package_file_path.exists() or not package_file_path.is_file():
            raise FileNotFoundError("Could not find package.json file")
    except Exception as exc:
        raise Exception(f"Invalid project template @ {template_dir!r}") from exc


def tmp_project(target_location: Path, template: Path | None = None) -> Project:
    """
    Creates a new project directory at the target using an optional template directory

    :param target_location: Target directory location to create the project directory in (`<target_location>/.tmp/project`)
    :param template: (Optional) directory whos content is copied and used as a template for the new project directory
    :return: ``Project`` object pointing to the temporary project directory
    """
...
def _module_project(module_path: Path) -> Project:
    _logger.debug(f"Creating project for module {module_path!r}")
    project_template_path = module_path / _PROJECT_TEMPLATE_REL_PATH
    return tmp_project(module_path, project_template_path)
    ...
    while curr_dir.name != "tests":
        if curr_dir == curr_dir.parent:
            raise FileNotFoundError(
                f"No directory test root directory 'tests' on path {request_path!r}"
            )
...
def project(request: FixtureRequest) -> Project:
    ...
    if resolution_mode == "global" or resolution_mode == "default":
        ...
        try:
            ...
            raise FileNotFoundError(f"Missing default test project for module @ {root_mod_dir.parent!r}") from err
...
@pytest.fixture(scope="session")
def cds_test_data(request: FixtureRequest) -> tuple[Path, Path]:
    try:
    ...
    test_data_path = request.config.rootpath / ".tmp" / "mii_cds_test_data"
...
@pytest.fixture
def struct_def(request: FixtureRequest, package_manager: FhirPackageManager) -> IdxStructureDefinition:
    """
    ...
    match request.param:
        ...
        case IdxStructureDefinition() as struct_def:
            return struct_def
        case pathlib.Path() as path:
            if not path.is_absolute():
                path = request.path.parent / "resources" / path
            with path.open(mode="r", encoding="utf-8") as file:
                struct_def = IdxStructureDefinition.model_validate_json(file.read())
            return struct_def
        case _ as param:
            raise ValueError(
                f"Fixture 'struct_def' does not support indirect parameters of type {type(param)} [supported_types=["
                f"{str!r}, {IdxStructureDefinition!r}, {pathlib.Path!r}]]"
            )
...
@pytest.fixture
def nav_struct_def(request, package_manager: FhirPackageManager) -> NavStructureDefinition:
    """
    Fixture that can be used for indirect parameters named 'nav_struct_def' to retrieve a `NavStructureDefinition`
    object for a `string` value representing the URL of the structure definition
    """
    match request.param:
        case str(profile_url):
            return package_manager.find_struct_def(profile_url)
        case pathlib.Path() as path:
            if not path.is_absolute():
                path = request.path.parent / "resources" / path
            with path.open(mode="rb") as file:
                struct_def = NavStructureDefinition.model_validate_json(file.read())
            return struct_def
        case NavStructureDefinition() as struct_def:
            return struct_def
        case StructureDefinition() as struct_def:
            return ensure_struct_def_is_navigable(struct_def)
        case _ as param:
            raise ValueError(
                f"Fixture 'nav_struct_def' does not support indirect parameters of type {type(param)} "
                f"[supported_types=[{str!r}, {NavStructureDefinition!r}]]"
            )


@pytest.fixture
def nav_elem_def(request, nav_struct_def) -> NavElementDefinition:
    """
    Fixture that can be used for indirect parameters named 'nav_elem_def' to retrieve a `NavElementDefinition` object
    for a `string` value representing the ID of the element definition
    """
    match request.param:
        case str(elem_id):
            return nav_struct_def.get_element_by_id(elem_id)
        case NavElementDefinition() as elem_def:
            return elem_def
        case _ as param:
            raise ValueError(
                f"Fixture 'elem_def' does not support indirect parameters of type {type(param)} "
                f"[supported_types=[{str!r}, {NavElementDefinition!r}]]"
            )
