...
from pydantic import PrivateAttr, computed_field, Field
...
class NavElementDefinition(ElementDefinition):
    ...
    _sub_elems_by_id: Mapping[str, "NavElementDefinition"] = PrivateAttr(default_factory=dict)
    _slices_by_name: Mapping[str, "NavElementDefinition"] = PrivateAttr(default_factory=dict)

    parent: Optional["NavElementDefinition"] = Field(default=None)

    @classmethod
    def construct_from(cls, struct_def: IdxStructureDefinition, elem_def: ElementDefinition,
                       realm: Optional[List[ElementDefinition]] = None,
                       parent: Optional["NavElementDefinition"] = None) -> "NavElementDefinition":
        """
    ...
    def _model_post_init(self, struct_def: IdxStructureDefinition, parent: Optional["NavElementDefinition"] = None, realm: Optional[List[ElementDefinition]] = None) -> None:
        """
        ...
        self.parent = parent
        if not realm:
            realm = self._struct_def.snapshot.element if self._struct_def.snapshot else self._struct_def.differential.element
        sub_realm = []
        sub_elems = []
        slices = []
        ...
        self._sub_elems_by_id = {key: self.construct_from(self._struct_def, ed, sub_realm, self) for key, ed in sub_elems}
        self._slices_by_name = {key: self.construct_from(self._struct_def, ed, sub_realm, self) for key, ed in slices}

    @property
    def struct_def(self) -> IdxStructureDefinition:
        return self._struct_def

    def element(self, rel_id: Optional[str] = None, full_id: Optional[str] = None) -> Optional[
    ...
    def slice(self, name: Optional[str] = None, full_id: Optional[str] = None) -> Optional[
        "NavElementDefinition"]:
        """
    ...
    def sub_elem_def(self, rel_id: Optional[str] = None, full_id: Optional[str] = None) -> Optional[
        "NavElementDefinition"]:
        """
