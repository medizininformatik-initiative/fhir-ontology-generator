...
class IdxStructureDefinitionDifferential(IdxStructureDefinition):
    def indexed_field_path(self) -> str:
        return "differential.element"


class IdxStructureDefinitionSnapshot(IdxStructureDefinition):
    def indexed_field_path(self) -> str:
...
def _idx_struct_def_discriminator_tag_value(v):
    if isinstance(v, dict):
        ...
        if isinstance(snapshot, dict):
            is_snapshot = len(snapshot.get("element", [])) > 0
        else:
            is_snapshot = len(getattr(snapshot, "element", [])) > 0
    else:
...
def idx_struct_def_discriminator(v: Any) -> Type:
    if isinstance(v, dict):
        ...
        is_snapshot = len(getattr(getattr(v, "snapshot", {}), "element", [])) > 0
    return (
        IdxStructureDefinitionSnapshot if is_snapshot else IdxStructureDefinitionDifferential
    )
...
IdxStructureDefinitionTA = TypeAdapter(
    Annotated[
        Union[
            Annotated[IdxStructureDefinitionDifferential, Tag("differential")],
            Annotated[IdxStructureDefinitionSnapshot, Tag("snapshot")],
        ],
