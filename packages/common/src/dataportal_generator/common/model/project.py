...
from dataportal_generator.common.log.functions import configure_logging, get_logger
...
class Project(ProjectDir):
    ...
    @classmethod
    def any_of(
        cls, name: str | None = None, path: str | Path | None = None
    ) -> tuple[str, Path, Mapping[str, str], ProjectConfig]:
        ...
        for fp in path.iterdir():
            if fp.is_file() and fp.stem == "config":
                with fp.open(mode="rb") as config_f:
                    match fp.suffix:
                        case ".yml" | ".yaml":
                            project_config = yaml.load(config_f, Loader=Loader)
                        case ".toml":
                            project_config = tomllib.load(config_f)
                break
        if not project_config:
            project_config = ProjectConfig()
        return name, path, env, project_config

    def __init__(self, name: str | None = None, path: str | Path | None = None):
    ...
    def included_profiles(
        ...
        pattern = {"resourceType": "StructureDefinition", "kind": "resource"}
        return [
            res
            for pkg, res in self.package_manager.iterate_cache(
                index_pattern=pattern, latest_only=latest_only, skip_on_fail=True
            )
