...
from dataportal_generator.common.util.http.client import BaseClient
...
def _version_matches(fuzzy: str, exact: str) -> bool:
    ...
    if fuzzy_split[1] == "x" and fuzzy_split[2] != "x":
        raise ValueError(f"Fuzzy version minor part cannot be 'x' if patch part is exact: {fuzzy!r}")
    # Clean from label from exact
    exact_split = exact.split("-", maxsplit=1)[0].split(".")[:3]
    if fuzzy_split[0] != exact_split[0]:
        return False
    if fuzzy_split[1] != "x" and fuzzy_split[1] != exact_split[1]:
        return False
    return not (fuzzy_split[2] != "x" and fuzzy_split[2] != exact_split[2])
...
class FhirPackageManager(abc.ABC):
    ...
    def _update_index_with_package(self, package_dir: Path):
        package = FhirPackage.from_directory(package_dir / "package", update_index_file=False)
        name_entry = self._index[package.name]
        if package.version not in name_entry:
            name_entry[package.version] = package
    ...
    def has_package(self, name: str, version: str | None) -> bool:
        """
    ...
    def iterate_cache(
        self,
        package_pattern: Mapping[str, Any] | None = None,
        index_pattern: Mapping[str, Any] | None = None,
        latest_only: bool = True,
        skip_on_fail: bool = False,
    ) -> Iterator[tuple[FhirPackage, FHIRAbstractModel]]:
        """
        ...
        """
        packages = filter(
            lambda p: package_pattern is None or p >= package_pattern,
            [
                package
        ...
        for package in packages:
            pass
            for file_entry, data in package.iter_raw(**index_pattern):
                res_type = file_entry.resourceType
                file_path = package.path / file_entry.filename
                try:
                    if res_type == "StructureDefinition":
                        res = construct_model(NavStructureDefinition, **json.loads(data))
                    else:
                        model_class = fhir.resources.R4B.get_fhir_model_class(res_type)
                        res = model_class.model_validate_json(data)
                    yield package, res
                except Exception as exc:
                    msg = f"Failed to load data @ {file_path}"
                    if skip_on_fail:
                        _logger.warning(f"{msg} => Skipping entry")
                        _logger.debug("Details:", exc_info=exc)
                    else:
                        raise Exception(f"Failed to load data @ {file_path}") from exc
    ...
    def find_struct_def(self, url: str) -> NavStructureDefinition | None:
        """
        Attempts to find a matching FHIR StructureDefinition in the package cache
    ...
    def dependencies_of(
        self,
        profile: str | StructureDefinition,
        package_pattern: Mapping[str, Any] | None = None,
        latest_only: bool = True,
    ) -> list[StructureDefinition]:
        """
...
class FirelyPackageManager(FhirPackageManager):
    ...
    def __handle_exception(
        ...
        match cause:
            ...
            case _ as exc:
                info = f"{msg}. Reason: {exc!s}"
        match level:
            case "warning":
                _logger.warning(info)
            case "error":
                _logger.warning(info, exc_info=cause)
            case "exception":
    ...
    def __init__(
        ...
        if version < self.MIN_SUPPORTED_VERSION:
            raise Exception(
                f"Installed 'firely.terminal' tool version is not supported [installed: {version!r}, required: "
                f"'>={self.MIN_SUPPORTED_VERSION}']"
            )
            ...
            for p in packages:
                match p:
                    ...
                    case _:
                        name_and_version = p
                try:
                    _logger.info(f"Installing package {name_and_version}")
                    subprocess.check_output(
...
class RepositoryPackageManager(FhirPackageManager):
    ...
    def inflate_cache(self, force: bool = False):
        if not force and not self.__inflated and self.__inflated_file_path.exists():
                with self.__inflated_file_path.open(mode="r", encoding="utf-8") as f:
                    self.__inflated = set(json.load(f))
        ...
        if force or self.__inflated != packages:
            _logger.info("Inflating package cache")
    ...
    def _request_package(
        self,
        name_and_version: tuple[str, str] | None = None,
        package_name: str | None = None,
        package_path: str | None = None,
        full_url: str | None = None,
    ) -> AbstractContextManager[Request]:
        """
                ...
                match p:
                    ...
                    case _:
                        raise UnsupportedError(
                            "A package version has to be provided in this implementation"
                        )
                if self.has_package(name, version):
                    _logger.debug(f"Package {name_and_version} is already installed")
                    continue
                try:
                    _logger.info(f"Installing package {name_and_version}")
                    ...
                    with self._request_package((name, version)) as response:
                        ...
                        with tarfile.open(package_tgz, mode="r:*") as tgz_file:
                            tgz_file.extractall(tmp_package_dir, filter="data")
                    ...
                    if deps:
                        _logger.debug(f"Installing (missing) dependencies of package {name_and_version}: {deps}")
                        for dep in deps:
                            try:
    ...
    def restore(self, inflate: bool = False, lenient: bool = False):
        ...
        if packages:
            _logger.info(
                f"Restoring (missing) package dependencies {', '.join(t[0] + '@' + t[1] for t in packages)}"
            )
            for pkg in packages:
                try:
            ...
            _logger.info("All dependencies are already present")
        if inflate:
            self.inflate_cache()
        self._update_index()
...
class GitHubPackageManager(RepositoryPackageManager):
    ...
    def _request_package(
        ...
        if exact_match:
            ...
            closest_match = max(fuzzy_matches, key=lambda e: e[0] + "-" + e[1])
            _logger.debug(
