...
from dataportal_generator.common.fhirpath.functions import (
    get_fhirpath_expression_root_node,
)
...
class OfTypeFunctionInvocation(FunctionInvocation):
    def __init__(self, invocation: fhirpathParser.FunctionInvocationContext):
        super().__init__(invocation)
        self.type_name = self._parameters[0].term().getText()
...
def _get_element_def_fuzzy(
    structure_def: NavStructureDefinition, element_def_id
) -> NavElementDefinition:
...
class FHIRPathResolver:
    """
    FHIRPath expression resolver returning a chain of tuples of `StructureDefinition` and `NavElementDefinition`s
    representing definition context (e.g. the `StructureDefinition`) and the corresponding partial path of the
    expression representing an `NavElementDefinition` that is defined by within this context
    """
    ...
    def __handle_possible_content_references(
    ...
    ) -> tuple[NavStructureDefinition, str]:
        ...
        if result := _find_definition_reference(elem_def):
            ...
            if type_code == "Extension":
                ...
                ref_struct_def = self.__package_manager.find_struct_def(definition_url)
    ...
    def __process_slice_function(
        ...
        structure_def: NavStructureDefinition,
        ...
        if (
            ...
            structure_def = self.__package_manager.find_struct_def(event.structure_ref)
            if not structure_def:
                raise Exception(
                    f"No such profile in context [structure_def={event.structure_ref!r}]"
                )
        ...
        try:
            ...
            raise Exception(
                f"No element definition for slice [event={event!r}]"
                + (
    ...
    def __process_resolve_function(
        self, structure_def: NavStructureDefinition, elem_def: NavElementDefinition
    ) -> NavStructureDefinition:
        ...
        match type_def.targetProfile:
            case [profile_url]:
                return self.__package_manager.find_struct_def(profile_url)
            case _:
                raise Exception(
    ...
    def resolve_path(
        self, context: NavStructureDefinition, fhir_path: str
    ) -> list[tuple[NavStructureDefinition, NavElementDefinition]]:
        """
                ...
                case FunctionInvocation():
                    match event:
                        case SliceFunctionInvocation():
                            s_sd, _, s_edi = self.__process_slice_function(
    ...
    def resolve_leaf(
        self, context: NavStructureDefinition, fhir_path: str
    ) -> tuple[NavStructureDefinition, NavElementDefinition] | None:
        """
