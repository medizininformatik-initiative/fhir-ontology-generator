...
from dataportal_generator.common.model.fhir.pydantic import validate_partial_model
from dataportal_generator.common.model.pydantic.mixins import CamelCase
...
def slices(pattern: str | None = None) -> str:
    return rf":{pattern}" if pattern else r":[a-zA-Z0-9\/\-_\[\]@]+"


def slicesOrSelf(pattern: str | None = None) -> str:
...
class FieldConfigRecommendation(BaseModel):
    ...
    def is_recommended(self, elem_def: ElementDefinition) -> bool | None:
...
class FieldConfig(BaseModel, ABC):
    ...
    def is_required(self, elem_def: ElementDefinition) -> bool | None:
        if self.required:
            return any(e.matches(elem_def) for e in self.required)
        else:
            return None

    def is_included(self, elem_def: ElementDefinition) -> bool | None:
...
class FieldsConfig(BaseModel):
    ...
    def is_recommended(
        ...
        matching_urls = filter(lambda url: url in self.profiles, profile_urls)
