...
from dataportal_generator.feature_selection.core.generators.profile_detail import (
    ProfileDetailGenerator,
)
from dataportal_generator.feature_selection.core.generators.profile_tree import (
...
def generate_r_load_sql(profile_details):
    with open(
        ...
        for index, profile_detail in enumerate(profile_details):
            profile_detaildb = profile_detail.model_dump_json(exclude_none=True).replace("'", "''")
...
if __name__ == "__main__":
    ...
    profile_tree = tree_generator.generate_profile_tree()
    ...
    if args.generate_profile_details:
        ...
        profile_details = (
            profile_detail_generator.generate_profile_details_for_profile_tree(
                profile_tree,
            )
        )
