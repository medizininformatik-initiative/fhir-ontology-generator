        ...
        if fds := profile_details.get(source_profile_url):
            ...
            for strat in group.stratifier:
                ...
                if fds.get(strat_coding.code):
                    ...
                    _logger.debug(
                        f"No field details exists for stratifier '{strat_coding.code}' => Dropping stratifier from group"
                    )
                    continue
...
def main():
    """
    Entry point used both by ``python -m`` / direct execution and by the ``generate-dse-element-availability``
