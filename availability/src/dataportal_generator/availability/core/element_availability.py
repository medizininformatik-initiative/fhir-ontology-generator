...
from dataportal_generator.common.fhirpath.functions import get_symbol
...
def _find_discr_value_defining_elem_def(
    ...
    :param struct_def: FHIR ``StructureDefinition`` defining the element
    ...
    if not root_elem_def:
        root_elem_def = struct_def.get_element_by_id(struct_def.type)
    ...
    while len(chain) > 0:
        ...
        if discr_elem_def := struct_def.get_element_by_id(discr_elem_def_id):
            elem_def = discr_elem_def
        else:
            raise NotFoundError(
                f"Missing value-defining element definition '{discr_elem_def_id}' for slice '{slice_name}' in "
                f"structure definition '{struct_def.url}' => Cannot generate filter"
            )
    return elem_def
...
class MeasureGroupGenerator:
    def __init__(self, package_manager: FhirPackageManager):
        ...
        self.__expr_cache: Dict[str, str] = dict()
    ...
    def _append_filter_from_profile_discriminated_elem(
        ...
        :param struct_def: Structure definition containing the discriminated element
        :return: Extended FHIRPath expression
        """
        types = elem.type
        if len(types) > 1:
            raise ValueError(
                f"Profile discriminated element '{elem.id} in structure definition '{struct_def.url}' supports more "
                f"than one type"
            )
        t = types[0]
        match t.code:
                ...
                clause = " or ".join(
                    [
                        ...
                        for url in [
                            ...
                            *[
                                p.url
                                for p in self.__package_manager.dependents_of(
            ...
            case _:
                raise UnsupportedError(
                    ...
                    f"elements [element_id='{elem.id}', profile_url='{struct_def.url}']"
                )
    ...
    def _get_filter_from_pattern_or_value_discriminated_elem(
        ...
        :param struct_def: Structure definition containing the element definition
        ...
        if "resolve()" in discr_path:
            ...
            ref_elem_def = (
                struct_def.get_element_by_id(f"{elem_def.id}.{pre_resolve}")
                if pre_resolve
                else elem_def
            )
            exprs = []
            for target_profile_url in ref_elem_def.type[0].targetProfile:
                target_profile = self.__package_manager.find(
            ...
            if discr_path != "$this":
                elem_def = _find_discr_value_defining_elem_def(
                    discr_path, struct_def, elem_def
                )
            if discr_val_t := _find_value_for_discriminator_pattern_or_value(elem_def):
                ...
                raise NotFoundError(
                    f"Missing any of fixed[x], pattern[x], or binding in element definition '{elem_def.id}' of "
                    f"structure definition '{struct_def.url}' targeted by the discriminator => Cannot generate filter"
                )
    ...
    def _append_filter_for_slice(
        ...
        :param struct_def: Structure definition contain the sliced element
        :return: Extended version of the given FHIRPath expression
        """
        parent_elem = get_parent_element(struct_def, slice_elem_def)
            ...
            match discr.type:
                case "value" | "pattern":
                    ...
                    if t.code == "Extension":
                        ...
                        if discr_path != "url":
                            raise UnsupportedError(
                                ...
                                f"[element_id='{slice_elem_def.id}', profile_url='{struct_def.url}']"
                            )
                        if t.profile:
                            checks = []
                            if len(t.profile) == 1:
                            ...
                            for url in t.profile:
                                ...
                                ext_snapshot = self.__package_manager.find(
                                    index_pattern
                                )
                            ...
                            where_filter = self._get_filter_from_pattern_or_value_discriminated_elem(
                                slice_elem_def, discr_path, struct_def
                            )
                    ...
                    return (
                        base_expr
                        + "."
                        + self._get_filter_from_pattern_or_value_discriminated_elem(
                            slice_elem_def,
                            discr_path,
                            struct_def,
                        )
                ...
                case "profile":
                    if discr_path == "$this":
                        ...
                        target_elem = struct_def.get_element_by_id(target_elem_id)
                    if not target_elem:
                        raise NotFoundError(
                            f"Could not find element definition '{target_elem_id}' in structure definition "
                            f"'{struct_def.url}' representing profile discriminated element"
                        )
                    return self._append_filter_from_profile_discriminated_elem(
                        base_expr, target_elem, discr, struct_def
                    )
                case _ as t:
                    raise Exception(
    ...
    def _generate_stratifier_for_reference(
        ...
        resolved_profiles = sorted(
            [
                ref_profile,
                *self.__package_manager.dependents_of(
                    ref_profile.url, MII_CDS_PACKAGE_PATTERN
                ),
            ],
            key=lambda p: p.url,
        )
    ...
    def _generate_stratifiers_for_extension_elements(
        self,
        ext_struct_def: IdxStructureDefinition,
        base_ext_expr: str,
        chained_elem_id: List[str],
    ) -> List[MeasureGroupStratifier]:
        """
        ...
        :param ext_struct_def: Structure definition instance defining the extension
        ...
        for elem_def in relevant_elem_defs:
            stratifiers.extend(
                self._generate_stratifiers_for_elem_def(
                    elem_def, ext_struct_def, expr_cache, chained_elem_id
                )
            )
        return stratifiers
            ...
            case "Extension":
                ...
                if elem_type.profile:
                    ...
                    for url in profile_urls:
                        index_pattern = {"url": url}
                        snapshot = self.__package_manager.find(index_pattern)
                        if not snapshot or not isinstance(
                            snapshot, StructureDefinition
                        ):
                            raise NotFoundError(
                                f"Could not find StructureDefinition resource defining extension structure '{url}'"
                            )
                        snapshot = IdxStructureDefinition.model_validate(snapshot)
                        stratifiers.extend(
    ...
    def _resolve_supported_types(
        ...
        if elem_def.type:
            ...
            if content_ref_profile_url and len(content_ref_profile_url) > 0:
                ...
                content_ref_profile = self.__package_manager.find(index_pattern)
    ...
    def _generate_stratifiers_for_elem_def(
        ...
        base_expr = self.__expr_cache.get(parent_elem_id)
        if not base_expr:
            raise KeyError(
        ...
        if elem_def.sliceName:
            expr = self._append_filter_for_slice(base_expr, elem_def, struct_def)
        else:
        ...
        if elem_def.max == "0":
            ...
            self.__expr_cache[elem_def.id] = expr
            return stratifiers
        # Handle type range
    ...
    def generate(
        ...
        :param struct_def: Structure definition to generated measure group for
        ...
        if not subject_ref_elem_def:
            raise MissingSubjectRefError(
                f"Structure definition '{struct_def.url}' has no suitable subject reference element and is thus no "
                f"measure group can be generated"
            )
        ...
        for elem_def in sorted(struct_def.snapshot.element, key=lambda ed: ed.id):
            if elem_def.id == struct_def.type or elem_def.id.endswith(".id"):
                # Skip root and ID element processing
                self.__expr_cache[elem_def.id] = elem_def.path
                continue
            ...
            try:
                ...
                _logger.warning(
                    f"Failed to generate stratifiers for element '{elem_def.id}' in structure definition "
                    f"'{struct_def.url}' => Skipping"
                )
...
class ElementAvailabilityGenerator:
    def __init__(self, project: Project):
        self.__project = project
        self.__package_manager = self.__project.package_manager

    def generate_measure(self, **elements) -> Measure:
        """
        ...
        for struct_def in self.__package_manager.iterate_cache(
            MII_CDS_PACKAGE_PATTERN, content_pattern, skip_on_fail=True
        ):
            if struct_def.type in ["SearchParameter"]:
                continue
            _logger.debug(
                f"Generating measure group for structure definition '{struct_def.url}'"
            )
            try:
                if (
                    ...
                    _logger.debug(
                        f"Structure definition '{struct_def.url}' is not in snapshot form => Skipping"
                    )
                    continue
                group_id_num = next(counter)
                struct_def = IdxStructureDefinition.model_validate(struct_def)
                measure_groups.append(
                    MeasureGroupGenerator(self.__package_manager).generate(
                        struct_def, group_id_num
                    )
                )
            except Exception as exc:
                match exc:
                    case MissingSubjectRefError():
                        _logger.info(
                            f"No eligible subject reference can identified in profile '{struct_def.url}' => Excluding"
                        )
                    case _:
                        _logger.error(
