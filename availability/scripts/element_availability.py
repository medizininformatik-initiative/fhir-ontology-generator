...
def _configure_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--project",
        required=True,
        help="Project to generate element availability for. The measure will include the profiles it depends on",
    )
    return parser
...
def _ensure_trailing_exists(expr_str: str) -> str:
    """
    Ensure that a FHIRPath expression is terminated by an exists function invocation. This is done by either append such
    an invocation pr by replacing a trailing where function invocation with an exists function invocation that has same
    parameters

    :param expr_str: FHIRPath expression string
    :return: Expression string with trailing exists function invocation
    """
    ...
    if isinstance(fc, fhirpathParser.FunctionContext):
        match get_symbol(fc.identifier()):
            ...
            case "where":
                ...
                plc_start = plc.start.start
                plc_end = plc.stop.stop
                param_str = expr_str[plc_start : plc_end + 1]
                return expr_str[:fc_start] + f"exists({param_str})"
    return expr_str + ".exists()"
...
def _append_filter_from_profile_discriminated_elem(
    ...
    match t.code:
        case "Reference" | "canonical":
            ...
            return f"{base_expr}.where({discr.path}.resolve().meta.profile.exists({clause}))"
        case "Extension":
            if len(t.profile) == 1:
        ...
        match discr.type:
            ...
            case "profile":
                target_elem = (
                    slice_elem_def
                    if discr_path == "$this"
                    else snapshot.get_element_by_id(f"{slice_elem_def.id}.{discr_path}")
                )
                if not target_elem:
                    raise NotFoundError(
...
def _generate_stratifiers_for_extension(
    ...
    the value element of the extension it contains

    :param ext_snapshot: `StructureDefinition` instance defining the extension
    :param base_expr: FHIRPath expression serving as the base for the FHIRPath expression for the generated stratifiers
    ...
    """
    stratifiers = []
    base_ext_expr = base_expr + f".extension('{ext_snapshot.url}')"
    ...
    expr_cache = {"Extension": base_ext_expr}
    ...
    match elem_type.code:
        case "Extension":
            ...
            if elem_type.profile:
                ...
                for url in profile_urls:
                    ...
                    stratifiers.extend(
                        _generate_stratifiers_for_extension(
                            ...
                            parent_expr,
                            manager,
                            [*chained_elem_id, parent_elem_id],
                        )
                    )
            else:
                ...
                stratifiers.append(
                    _generate_stratifier(
                        _ensure_trailing_exists(parent_expr),
        ...
        case _ as type_code:
            ...
            return [
                _generate_stratifier(
                    _ensure_trailing_exists(expr),
                    _get_full_element_id(chained_elem_id, type_code),
                )
            ]
...
def _generate_stratifier_ids(measure: Measure) -> Measure:
    """
...
def generate_measure(manager: FhirPackageManager) -> Measure:
    measure = Measure(
        id="CdsElementAvailabilityMeasure",
        meta=Meta(
            profile=[
    ...
    for profile in manager.iterate_cache(
        ...
        try:
            ...
            _logger.debug("Details:", exc_info=exc)
    return _generate_stratifier_ids(measure)
...
if __name__ == "__main__":
    ...
    _logger.info("Generating measure")
    element_measure = generate_measure(project.package_manager)

    measure_file_path = (
        project.output.availability / "Measure-CdsElementAvailability.fhir.json"
    )
